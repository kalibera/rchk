...
[here](doc/BUILDING.md)).  For convenience of playing with the tools, the bitcode
of the R binary from R-devel version 67741 generated by CLANG/LLVM 4.2.0 is
included in the `examples` directory.

The tools need [LLVM 3.6.1](http://llvm.org/releases/download.html) to build
(one can download one of the CLANG/LLVM [pre-built
binaries](http://llvm.org/releases/download.html#3.6.1)).  One then needs to
set the LLVM installation path in `src/Makefile` of `rchk` and run `make`. 
LLVM API changes frequently, but it may be that `rchk` will also work with
newer 3.6.x releases when they come out.
